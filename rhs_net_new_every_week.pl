			...
			foreach (@test1)
			{
				...
				if ($test2 =~ /=/)
				{
					...
					if ($b =~ /opened/i)
					{
						...
						if ($yy <= $xx && $yy >= $zz)
						{
							# This bug was opened within the last 7 days
							$count++;
...
for ( $i = 0; $i <= $j; $i = $i+7 )
{
	...
	my $ooo = get_open_bug_count($dest);
	my $rrr = get_fix_bug_count($dest);
	#print ("$dest,$rrr,$ooo\n");
	my $sss = get_verified_bug_count($dest);
	print ("$dest,$rrr,$ooo,$sss\n");

	push @date_list , $dest;
	push @resolved_list , $rrr;
	push @open_list , $ooo;
	push @verified_list , $sss;
}
...
$mygraph->set(
    ...
    title       => 'Red Hat Storage - 2.1 - Bug Graph',
    bar_spacing => 1,
    bar_width   => 50,
    transparent => 0,
    x_labels_vertical => 1,
...
$mygraph->set_title_font($font_file, 20);
...
$mygraph->set_legend('NEW/ASSIGNED', 'MODIFIED', 'VERIFIED');

my $myimage = $mygraph->plot(\@data) or die $mygraph->error;

# write to file
open OUT, ">rhs_net_new_every_week.png";
binmode OUT;
